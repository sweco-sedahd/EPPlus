...
namespace OfficeOpenXml
{
    ...
    public class ExcelNamedRangeCollection : IEnumerable<ExcelNamedRange>
    {
        ...
        readonly List<ExcelNamedRange> _list = new();
        ...
        {
            IEnumerable<ExcelNamedRange> namedRanges = _list.Where(filter);
        ...
        private void InsertColumns(int colFrom, int cols, ExcelNamedRange namedRange)
        {
            if (colFrom > 0)
            {
                ...
                {
                    string newAddress = ExcelCellBase.GetAddress(namedRange.Start.Row, namedRange.Start.Column + cols, namedRange.End.Row, namedRange.End.Column + cols);
                ...
                {
                    string newAddress = ExcelCellBase.GetAddress(namedRange.Start.Row, namedRange.Start.Column, namedRange.End.Row, namedRange.End.Column + cols);
                    namedRange.Address = BuildNewAddress(namedRange, newAddress);
                }
            }
        }
        ...
        private static string BuildNewAddress(ExcelNamedRange namedRange, string newAddress)
        {
            ...
            {
                string worksheet = namedRange.FullAddress.Split('!')[0];
        ...
        private void InsertRows(int rowFrom, int rows, ExcelNamedRange namedRange)
        {
            if (rows > 0)
            {
                ...
                {
                    string newAddress = ExcelCellBase.GetAddress(namedRange.Start.Row + rows, namedRange.Start.Column, namedRange.End.Row + rows, namedRange.End.Column);
                    namedRange.Address = BuildNewAddress(namedRange, newAddress);
                }
                else if (rowFrom <= namedRange.End.Row && namedRange.End.Row + rows <= ExcelPackage.MaxRows)
                {
                    string newAddress = ExcelCellBase.GetAddress(namedRange.Start.Row, namedRange.Start.Column, namedRange.End.Row + rows, namedRange.End.Column);
                    namedRange.Address = BuildNewAddress(namedRange, newAddress);
                }
            }
        }
