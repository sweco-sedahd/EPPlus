...
namespace OfficeOpenXml.Drawing
{
    ...
    public class ExcelDrawings : IEnumerable<ExcelDrawing>, IDisposable
    {
        ...
        internal Dictionary<string, string> _hashes = new();
        internal ExcelPackage _package;

        internal ExcelDrawings(ExcelPackage xlPackage, ExcelWorksheet sheet)
        {
            DrawingXml = new XmlDocument();
            DrawingXml.PreserveWhitespace = false;
            ...
            if (node != null)
            {
                ...
                UriDrawing = UriHelper.ResolvePartUri(sheet.WorksheetUri, _drawingRelation.TargetUri);

                Part = xlPackage.Package.GetPart(UriDrawing);
                XmlHelper.LoadXmlSafe(DrawingXml, Part.GetStream());
        ...
        /// </summary>
        public XmlDocument DrawingXml { get; private set; } = new();
        ...
        public void Dispose()
        {
            ...
            foreach (ExcelDrawing d in _drawings)
            {
                d.Dispose();
            }
        ...
        private void AddDrawings()
        {
            ...
            XmlNodeList list = DrawingXml.SelectNodes("//*[self::xdr:twoCellAnchor or self::xdr:oneCellAnchor or self::xdr:absoluteAnchor]", NameSpaceManager);
        ...
        public Uri UriDrawing { get; private set; }
        ...
        public ExcelChart AddChart(string Name, eChartType ChartType, ExcelPivotTable PivotTableSource)
        {
            ...
            if (ChartType is eChartType.StockHLC or eChartType.StockOHLC or eChartType.StockVOHLC)
            {
        ...
        private XmlElement CreateDrawingXml()
        {
            if (DrawingXml.DocumentElement == null)
            {
                ...
                do
                {
                    UriDrawing = new Uri(string.Format("/xl/drawings/drawing{0}.xml", id++), UriKind.Relative);
                } while (package.PartExists(UriDrawing));

                Part = package.CreatePart(UriDrawing, "application/vnd.openxmlformats-officedocument.drawing+xml", _package.Compression);
                ...
                _drawingRelation = Worksheet.Part.CreateRelationship(UriHelper.GetRelativeUri(Worksheet.WorksheetUri, UriDrawing), TargetMode.Internal, ExcelPackage.schemaRelationships + "/drawing");
            ...
            XmlNode colNode = DrawingXml.SelectSingleNode("//xdr:wsDr", NameSpaceManager);
            XmlElement drawNode;
            if (Worksheet is ExcelChartsheet)
            {
                drawNode = DrawingXml.CreateElement("xdr", "absoluteAnchor", ExcelPackage.schemaSheetDrawings);
                XmlElement posNode = DrawingXml.CreateElement("xdr", "pos", ExcelPackage.schemaSheetDrawings);
                posNode.SetAttribute("y", "0");
                posNode.SetAttribute("x", "0");
                drawNode.AppendChild(posNode);
                XmlElement extNode = DrawingXml.CreateElement("xdr", "ext", ExcelPackage.schemaSheetDrawings);
            ...
            {
                drawNode = DrawingXml.CreateElement("xdr", "twoCellAnchor", ExcelPackage.schemaSheetDrawings);
                colNode.AppendChild(drawNode);
                //Add from position Element;
                XmlElement fromNode = DrawingXml.CreateElement("xdr", "from", ExcelPackage.schemaSheetDrawings);
                ...
                XmlElement toNode = DrawingXml.CreateElement("xdr", "to", ExcelPackage.schemaSheetDrawings);
        ...
        {
            ExcelDrawing draw = _drawings[Index];
            draw.DeleteMe();
