...
namespace OfficeOpenXml
{
    ...
    public sealed class ExcelWorkbook : XmlHelper, IDisposable
    {
            ...
            base(namespaceManager)
        {
            ...
            SchemaNodeOrder = new[] { "fileVersion", "fileSharing", "workbookPr", "workbookProtection", "bookViews", "sheets", "functionGroups", "functionPrototypes", "externalReferences", "definedNames", "calcPr", "oleSize", "customWorkbookViews", "pivotCaches", "smartTagPr", "smartTagTypes", "webPublishing", "fileRecoveryPr", "webPublishObjects", "extLst" };
                ...
                foreach (XmlElement elem in nl)
                {
                    ...
                    if (fullAddress.IndexOf("[") == 0)
                    {
                        ...
                        if (start >= 0 && end >= 0)
                        {
                            ...
                            if (int.TryParse(externalIndex, NumberStyles.Any, CultureInfo.InvariantCulture, out int index))
                            {
        ...
        internal void AddPivotTable(string cacheID, Uri defUri)
        {
            ...
            ZipPackageRelationship rel = Part.CreateRelationship(UriHelper.ResolvePartUri(WorkbookUri, defUri), TargetMode.Internal, ExcelPackage.schemaRelationships + "/pivotCacheDefinition");
        ...
        internal void GetExternalReferences()
        {
            ...
            if (nl != null)
            {
                foreach (XmlElement elem in nl)
                {
                    ...
                    ZipPackagePart part = _package.Package.GetPart(UriHelper.ResolvePartUri(rel.SourceUri, rel.TargetUri));
                    ...
                    if (xmlExtRef.SelectSingleNode("//d:externalBook", NameSpaceManager) is XmlElement book)
                    {
                        ...
                        ZipPackageRelationship rel_extRef = part.GetRelationship(rId_ExtRef);
                        if (rel_extRef != null)
                        {
        ...
        public decimal MaxFontWidth
        {
            get
            {
                int ix = Styles.NamedStyles.FindIndexByID("Normal");
                if (ix >= 0)
                {
        ...
        internal static decimal GetWidthPixels(string fontName, float fontSize)
        {
            ...
            float min = -1, max = 500;
            foreach (KeyValuePair<float, FontSizeInfo> size in font)
            {
        ...
        /// </summary>
        internal Uri WorkbookUri { get; }

        /// <summary>
        /// URI to the styles inside the package
        /// </summary>
        internal Uri StylesUri { get; }

        /// <summary>
        /// URI to the shared strings inside the package
        /// </summary>
        internal Uri SharedStringsUri { get; }
        ...
        private void CreateWorkbookXml(XmlNamespaceManager namespaceManager)
        {
            ...
            else
            {
                ...
                ZipPackagePart partWorkbook = _package.Package.CreatePart(WorkbookUri, @"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml", _package.Compression);
            ...
            get
            {
                if (_stylesXml == null)
                {
                    ...
                    else
                    {
                        ...
                        ZipPackagePart part = _package.Package.CreatePart(StylesUri, @"application/vnd.openxmlformats-officedocument.spreadsheetml.styles+xml", _package.Compression);
                        ...
                        _package.Workbook.Part.CreateRelationship(UriHelper.GetRelativeUri(WorkbookUri, StylesUri), TargetMode.Internal, ExcelPackage.schemaRelationships + "/styles");
                        _package.Package.Flush();
                    }
                }
        ...
        internal void Save() // Workbook Save
        {
            ...
            else
            {
                ...
                Part.CreateRelationship(UriHelper.GetRelativeUri(WorkbookUri, SharedStringsUri), TargetMode.Internal, ExcelPackage.schemaRelationships + "/sharedStrings");
            }
