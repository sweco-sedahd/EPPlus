...
namespace OfficeOpenXml
{
    ...
    public sealed class ExcelPackage : IDisposable
    {
        ...
        public ExcelPackage(FileInfo template, bool useStream)
        {
            ...
            if (useStream == false)
            {
                File = new FileInfo(Path.GetTempPath() + Guid.NewGuid() + ".xlsx");
            }
        }
        ...
        internal ImageInfo AddImage(byte[] image, Uri uri, string contentType)
        {
            ...
            lock (_images)
            {
                ...
                else
                {
                    ...
                    MemoryStream stream = imagePart.GetStream(FileMode.Create, FileAccess.Write);
        ...
        private void Init()
        {
            ...
            IConfigurationRoot c = build.Build();

            string v = c["EPPlus:ExcelPackage:Compatibility:IsWorksheets1Based"];
#else
        ...
        internal void SavePart(Uri uri, XmlDocument xmlDoc)
        {
            ...
            MemoryStream stream = part.GetStream(FileMode.Create, FileAccess.Write);
            ...
            try
            {
                ...
                if (File == null)
                {
                    if (Encryption.IsEncrypted)
                    {
                        ...
                        MemoryStream msEnc = eph.EncryptPackage(file, Encryption);
                ...
                else
                {
                    ...
                    if (Stream is MemoryStream)
                    {
                        ...
                        if (Encryption.IsEncrypted)
                        {
                            ...
                            var eph = new EncryptedPackageHandler();
                            MemoryStream ms = eph.EncryptPackage(file, Encryption);
        ...
        internal byte[] GetAsByteArray(bool save)
        {
            ...
            {
                var eph = new EncryptedPackageHandler();
                MemoryStream ms = eph.EncryptPackage(byRet, Encryption);
                byRet = ms.ToArray();
            }
