...
namespace OfficeOpenXml
{
    ...
    public class ExcelHeaderFooterText
    {
        ...
        public ExcelVmlDrawingPicture InsertPicture(Image Picture, PictureAlignment Alignment)
        {
            ...
            //Add the image
#if (Core)
            byte[] img = ImageCompat.GetImageAsByteArray(Picture);
#else
            ...
            ExcelPackage.ImageInfo ii = _ws.Workbook._package.AddImage(img);
        ...
        public ExcelVmlDrawingPicture InsertPicture(FileInfo PictureFile, PictureAlignment Alignment)
        {
            ...
            Uri uriPic = XmlHelper.GetNewUri(_ws._package.Package, "/xl/media/" + PictureFile.Name.Substring(0, PictureFile.Name.Length - PictureFile.Extension.Length) + "{0}" + PictureFile.Extension);
#if (Core)
            byte[] imgBytes = ImageCompat.GetImageAsByteArray(Picture);
#else
            var ic = new ImageConverter();
            byte[] imgBytes = (byte[])ic.ConvertTo(Picture, typeof(byte[]));
#endif

            ExcelPackage.ImageInfo ii = _ws.Workbook._package.AddImage(imgBytes, uriPic, contentType);
    ...
    public sealed class ExcelHeaderFooter : XmlHelper
    {
        ...
        {
            _ws = ws;
            SchemaNodeOrder = new[] { "headerFooter", "oddHeader", "oddFooter", "evenHeader", "evenFooter", "firstHeader", "firstFooter" };
        }
            ...
            if (_vmlDrawingsHF != null)
            {
                ...
                else
                {
                    ...
                    if (_vmlDrawingsHF.Part == null)
                    {
                        ...
                        ZipPackageRelationship rel = _ws.Part.CreateRelationship(UriHelper.GetRelativeUri(_ws.WorksheetUri, _vmlDrawingsHF.Uri), TargetMode.Internal, ExcelPackage.schemaRelationships + "/vmlDrawing");
                        _ws.SetHFLegacyDrawingRel(rel.Id);
                        _vmlDrawingsHF.RelId = rel.Id;
                        foreach (ExcelVmlDrawingPicture draw in _vmlDrawingsHF)
                        {
                            rel = _vmlDrawingsHF.Part.CreateRelationship(UriHelper.GetRelativeUri(_vmlDrawingsHF.Uri, draw.ImageUri), TargetMode.Internal, ExcelPackage.schemaRelationships + "/image");
                            draw.RelId = rel.Id;
                        }
                    }
