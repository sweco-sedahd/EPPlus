...
namespace OfficeOpenXml.Table.PivotTable
{
    ...
    public class ExcelPivotTableField : XmlHelper
    {
        ...
        /// </summary>        
        public ExcelPivotTableFieldGroup Grouping { get; private set; }
        ...
        {
            if (groupInterval is < 1 or >= short.MaxValue)
            {
        ...
        private ExcelPivotTableField AddField(eDateGroupBy groupBy, DateTime startDate, DateTime endDate, ref bool firstField, int interval)
        {
            ...
            {
                //Pivot field
                XmlNode topNode = _table.PivotTableXml.SelectSingleNode("//d:pivotFields", _table.NameSpaceManager);
                XmlElement fieldNode = _table.PivotTableXml.CreateElement("pivotField", ExcelPackage.schemaMain);
        ...
        private void AddCacheField(ExcelPivotTableField field, DateTime startDate, DateTime endDate, int interval)
        {
            ...
            XmlNode cacheTopNode = _table.CacheDefinition.CacheDefinitionXml.SelectSingleNode("//d:cacheFields", _table.NameSpaceManager);
            XmlElement cacheFieldNode = _table.CacheDefinition.CacheDefinitionXml.CreateElement("cacheField", ExcelPackage.schemaMain);
        ...
        internal void SetCacheFieldNode(XmlNode cacheField)
        {
            ...
            XmlNode groupNode = cacheField.SelectSingleNode("d:fieldGroup", NameSpaceManager);
            if (groupNode != null)
            {
                XmlNode groupBy = groupNode.SelectSingleNode("d:rangePr/@groupBy", NameSpaceManager);
                if (groupBy == null)
                {
                    Grouping = new ExcelPivotTableFieldNumericGroup(NameSpaceManager, cacheField);
                }
                else
                {
        ...
        private void AddFieldItems(int items)
        {
            ...
            {
                XmlElement itemNode = itemsNode.OwnerDocument.CreateElement("item", ExcelPackage.schemaMain);
