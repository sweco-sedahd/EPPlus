...
namespace OfficeOpenXml.Table.PivotTable
{
    ...
    public class ExcelPivotCacheDefinition : XmlHelper
    {
        ...
        internal ExcelPivotCacheDefinition(XmlNamespaceManager ns, ExcelPivotTable pivotTable) :
            base(ns, null)
        {
            foreach (ZipPackageRelationship r in pivotTable.Part.GetRelationshipsByType(ExcelPackage.schemaRelationships + "/pivotCacheDefinition"))
            {
                Relationship = r;
            }
            ...
            ZipPackage pck = pivotTable.WorkSheet._package.Package;
            ...
            {
                string worksheetName = GetXmlNodeString(_sourceWorksheetPath);
            ...
            base(ns, null)
        {
            ...
            ZipPackagePart recPart = pck.CreatePart(CacheRecordUri, ExcelPackage.schemaPivotCacheRecords);
            cacheRecord.Save(recPart.GetStream());

            RecordRelationship = Part.CreateRelationship(UriHelper.ResolvePartUri(CacheDefinitionUri, CacheRecordUri), TargetMode.Internal, ExcelPackage.schemaRelationships + "/pivotCacheRecords");
                ...
                if (_sourceRange == null)
                {
                    if (CacheSource == eSourceType.Worksheet)
                    {
                        ...
                        if (ws == null) //Not worksheet, check name or table name
                        {
                            ...
                            foreach (ExcelNamedRange n in PivotTable.WorkSheet.Workbook.Names)
                            {
                            ...
                            foreach (ExcelWorksheet w in PivotTable.WorkSheet.Workbook.Worksheets)
                            {
        ...
        private string GetStartXml(ExcelRangeBase sourceAddress)
        {
            ...
            ExcelWorksheet sourceWorksheet = PivotTable.WorkSheet.Workbook.Worksheets[sourceAddress.WorkSheet];
