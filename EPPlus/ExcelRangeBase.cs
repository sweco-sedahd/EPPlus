...
namespace OfficeOpenXml
{
    ...
    public class ExcelRangeBase : ExcelAddress, IExcelCell, IDisposable, IEnumerable<ExcelRangeBase>, IEnumerator<ExcelRangeBase>
    {
        ...
        {
            Sort(new[] { column }, new[] { descending });
        }
        ...
        public void Sort(int[] columns, bool[] descending = null, CultureInfo culture = null, CompareOptions compareOptions = CompareOptions.None)
        {
            ...
            Dictionary<string, byte> flags = GetItems(_worksheet._flags, _fromRow, _fromCol, _toRow, _toCol);
            Dictionary<string, object> formulas = GetItems(_worksheet._formulas, _fromRow, _fromCol, _toRow, _toCol);
            Dictionary<string, Uri> hyperLinks = GetItems(_worksheet._hyperLinks, _fromRow, _fromCol, _toRow, _toCol);
            Dictionary<string, int> comments = GetItems(_worksheet._commentsStore, _fromRow, _fromCol, _toRow, _toCol);
            ...
            for (int r = 0; r < l.Count; r++)
            {
                for (int c = 0; c < cols; c++)
                {
                    ...
                    string addr = GetAddress(l[r].Row, _fromCol + c);
                    //Move flags
                    ...
                    if (formulas.ContainsKey(addr))
                    {
                        ...
                        if (formulas[addr] is int)
                        {
                            ...
                            ExcelWorksheet.Formulas f = Worksheet._sharedFormulas[sfIx];
        ...
        private class Comp : IComparer<SortItem<ExcelCoreValue>>
        {
            ...
            public int Compare(SortItem<ExcelCoreValue> x, SortItem<ExcelCoreValue> y)
            {
                ...
                for (int i = 0; i < columns.Length; i++)
                {
                    object x1 = x.Items[columns[i]]._value;
                    object y1 = y.Items[columns[i]]._value;
                    bool isNumX = ConvertUtil.IsNumeric(x1);
                    bool isNumY = ConvertUtil.IsNumeric(y1);
        ...
        public void AutoFitColumns(double MinimumWidth, double MaximumWidth)
        {
            ...
            int[,] drawWidths = _worksheet.Drawings.GetDrawingWidths();
            ...
            {
                foreach (ExcelAddress addr in Addresses)
                {
            ...
            ExcelFontXml nf = styles.Fonts[styles.CellXfs[0].FontId];
            ...
            foreach (ExcelRangeBase cell in this)
            {
                ...
                SizeF size = g.MeasureString(t, f, 10000, StringFormat.GenericDefault);
        ...
        private void SetMinWidth(double minimumWidth, int fromCol, int toCol)
        {
            ...
            foreach (ExcelCoreValue val in iterator)
            {
                ...
                {
                    ExcelColumn newCol = _worksheet.Column(prevCol);
        ...
        private static string GetDateText(DateTime d, string format, ExcelNumberFormatXml.ExcelFormatTranslator nf)
        {
            ...
            if (nf.SpecialDateFormat == ExcelNumberFormatXml.ExcelFormatTranslator.eSystemDateFormat.SystemLongTime)
            {
                return d.ToLongTimeString();
            }

            if (nf.SpecialDateFormat == ExcelNumberFormatXml.ExcelFormatTranslator.eSystemDateFormat.SystemShortDate)
            {
        ...
        public bool AutoFilter
        {
            ...
            set
            {
                ...
                if (_worksheet.AutoFilterAddress != null)
                {
                    eAddressCollition c = Collide(_worksheet.AutoFilterAddress);
                    if (value == false && c is eAddressCollition.Partly or eAddressCollition.No)
                    {
                ...
                if (value)
                {
                    ...
                    ExcelNamedRange result = _worksheet.Names.Add("_xlnm._FilterDatabase", this);
        ...
        private ExcelRichTextCollection GetRichText(int row, int col)
        {
            ...
            object v = _worksheet.GetValueInner(row, col);
            bool isRt = _worksheet._flags.GetFlagValue(row, col, CellFlags.RichText);
            if (v != null)
            {
                if (isRt)
                {
                    XmlHelper.LoadXmlSafe(xml, "<d:si xmlns:d=\"http://schemas.openxmlformats.org/spreadsheetml/2006/main\" >" + v + "</d:si>", Encoding.UTF8);
                }
                else
                {
        ...
        public new string FullAddress
        {
            get
            {
                ...
                if (Addresses != null)
                {
                    foreach (ExcelAddress a in Addresses)
                    {
        ...
        {
            ExcelRangeBase r = LoadFromDataTable(Table, PrintHeaders);

            int rows = (Table.Rows.Count == 0 ? 1 : Table.Rows.Count) + (PrintHeaders ? 1 : 0);
            if (rows >= 0 && Table.Columns.Count > 0)
            {
                ExcelTable tbl = _worksheet.Tables.Add(new ExcelAddressBase(_fromRow, _fromCol, _fromRow + rows - 1, _fromCol + Table.Columns.Count - 1), Table.TableName);
                tbl.ShowHeader = PrintHeaders;
                tbl.TableStyle = TableStyle;
            }
        ...
        public ExcelRangeBase LoadFromDataTable(DataTable Table, bool PrintHeaders)
        {
            ...
            _worksheet._values.SetRangeValueSpecial(_fromRow, _fromCol, _fromRow + rowArray.Count - 1, _fromCol + Table.Columns.Count - 1,
                (list, index, rowIx, columnIx, value) =>
                {
                    ...
                    object val = ((List<object[]>)value)[rowIx][columnIx];
        ...
        public ExcelRangeBase LoadFromArrays(IEnumerable<object[]> Data)
        {
            ...
            _worksheet._values.SetRangeValueSpecial(_fromRow, _fromCol, _fromRow + rowArray.Count - 1, _fromCol + maxColumn - 1,
                (list, index, rowIx, columnIx, value) =>
                {
                    ...
                    object[] item = values[rowIx];
                    if (item.Length <= columnIx) return;

                    object val = item[columnIx];
                    if (val != null && val != DBNull.Value && !string.IsNullOrEmpty(val.ToString()))
                    {
                        list[index] = new ExcelCoreValue { _value = val, _styleId = list[index]._styleId };
                    }
        ...
        public ExcelRangeBase LoadFromCollection<T>(IEnumerable<T> Collection, bool PrintHeaders, TableStyles TableStyle, BindingFlags memberFlags, MemberInfo[] Members)
        {
            ...
            // create buffer
            object[,] values = new object[PrintHeaders ? Collection.Count() + 1 : Collection.Count(), Members.Length];
            ...
            ExcelRange r = _worksheet.Cells[_fromRow, _fromCol, _fromRow + row - 1, _fromCol + col - 1];
        ...
        {
            if (Format.EOL == null || Format.EOL.Length == 0) return new[] { text };
        ...
        public ExcelComment AddComment(string Text, string Author)
        {
            ...
            //Create the comments
            _changePropMethod(this, _setCommentDelegate, new[] { Text, Author });
        ...
        public void Copy(ExcelRangeBase Destination, ExcelRangeCopyOptionFlags? excelRangeCopyOptionFlags)
        {
            ...
            bool excludeFormulas = excelRangeCopyOptionFlags.HasValue && (excelRangeCopyOptionFlags.Value & ExcelRangeCopyOptionFlags.ExcludeFormulas) == ExcelRangeCopyOptionFlags.ExcludeFormulas;
            ...
            while (cses.Next())
            {
                ...
                {
                    int row = Destination._fromRow + (cses.Row - _fromRow);
                    int col = Destination._fromCol + (cses.Column - _fromCol);
                    var cell = new CopiedCell
                    {
                        Row = row,
                        Column = col,
            ...
            while (csem.Next())
            {
                if (!copiedMergedCells.ContainsKey(csem.Value))
                {
                    ...
                    eAddressCollition collideResult = Collide(adr);
                    if (collideResult is eAddressCollition.Inside or eAddressCollition.Equal)
                    {
            ...
            foreach (CopiedCell cell in copiedValue)
            {
            ...
            {
                eAddressCollition collideStatus = Collide(Worksheet.Dimension);
                if (collideStatus is not (eAddressCollition.Equal and eAddressCollition.Inside))
                {
            ...
            if (_fromCol == 1 && _toCol == ExcelPackage.MaxColumns)
            {
                ...
                {
                    ExcelRow destinationRow = Destination.Worksheet.Row(Destination.Start.Row + r);
            ...
            if (_fromRow == 1 && _toRow == ExcelPackage.MaxRows)
            {
                ...
                {
                    ExcelColumn destinationCol = Destination.Worksheet.Column(Destination.Start.Column + c);
        ...
        internal void Delete(ExcelAddressBase Range, bool shift)
        {
            ...
            if (Addresses != null)
            {
                foreach (ExcelAddress sub in Addresses)
                {
                    Delete(sub, shift);
                }
            }
        ...
        private void DeleteCheckMergedCells(ExcelAddressBase Range)
        {
            ...
            foreach (string addr in Worksheet.MergedCells)
            {
                eAddressCollition addrCol = Range.Collide(new ExcelAddress(Range.WorkSheet, addr));
        ...
        /// The current range when enumerating
        /// </summary>
        object IEnumerator.Current => new ExcelRangeBase(_worksheet, GetAddress(cellEnum.Row, cellEnum.Column));
