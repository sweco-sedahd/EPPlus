...
namespace OfficeOpenXml
{
    ...
    public class ExcelCommentCollection : IEnumerable, IDisposable
    {
        ...
        readonly List<ExcelComment> _list = new();
        ...
        {
            ZipPackageRelationshipCollection commentParts = Worksheet.Part.GetRelationshipsByType(ExcelPackage.schemaComment);
            bool isLoaded = false;
            CommentXml = new XmlDocument();
            foreach (ZipPackageRelationship commentPart in commentParts)
            {
        ...
        {
            ulong id = ExcelCellBase.GetCellID(Worksheet.SheetID, comment.Range._fromRow, comment.Range._fromCol);
