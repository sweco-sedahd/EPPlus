    ...
    public class ExcelBackgroundImage : XmlHelper
    {
        ...
        public Image Image
        {
            get
            {
                ...
                {
                    ZipPackageRelationship rel = _workSheet.Part.GetRelationship(relID);
                    ZipPackagePart imagePart = _workSheet.Part.Package.GetPart(UriHelper.ResolvePartUri(rel.SourceUri, rel.TargetUri));
            ...
            set
            {
                ...
                {
#if (Core)
                    byte[] img = ImageCompat.GetImageAsByteArray(value);
#else
                    ImageConverter ic = new ImageConverter();
                    byte[] img = (byte[])ic.ConvertTo(value, typeof(byte[]));
#endif
                    ExcelPackage.ImageInfo ii = _workSheet.Workbook._package.AddImage(img);
                    ZipPackageRelationship rel = _workSheet.Part.CreateRelationship(ii.Uri, TargetMode.Internal, ExcelPackage.schemaRelationships + "/image");
        ...
        public void SetFromFile(FileInfo PictureFile)
        {
            ...
            Uri imageURI = GetNewUri(_workSheet._package.Package, "/xl/media/" + PictureFile.Name.Substring(0, PictureFile.Name.Length - PictureFile.Extension.Length) + "{0}" + PictureFile.Extension);

            ExcelPackage.ImageInfo ii = _workSheet.Workbook._package.AddImage(fileBytes, imageURI, contentType);
            ...
            ZipPackagePart imagePart = _workSheet.Part.Package.CreatePart(imageURI, contentType, CompressionLevel.None);
            //Save the picture to package.

            MemoryStream strm = imagePart.GetStream(FileMode.Create, FileAccess.Write);
            strm.Write(fileBytes, 0, fileBytes.Length);

            ZipPackageRelationship rel = _workSheet.Part.CreateRelationship(imageURI, TargetMode.Internal, ExcelPackage.schemaRelationships + "/image");
            SetXmlNodeString(BACKGROUNDPIC_PATH, rel.Id);
        }
...
#endif
                ExcelPackage.ImageInfo ii = _workSheet.Workbook._package.GetImageInfo(img);
