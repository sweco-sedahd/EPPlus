...
namespace OfficeOpenXml.VBA
{
    ...
    public class ExcelVbaSignature
    {
        ...
        readonly ZipPackagePart _vbaPart;
        ...
        private void GetSignature()
        {
            ...
            ZipPackageRelationship rel = _vbaPart.GetRelationshipsByType(schemaRelVbaSignature).FirstOrDefault();
            if (rel != null)
            {
        ...
        internal void Save(ExcelVbaProject proj)
        {
            ...
            {
                X509Certificate2 storeCert = GetCertFromStore(StoreLocation.CurrentUser);
                if (storeCert == null)
                {
            ...
            ZipPackageRelationship rel = proj.Part.GetRelationshipsByType(schemaRelVbaSignature).FirstOrDefault();
            if (Part == null)
            {
                if (rel != null)
                {
            ...
            if (rel == null)
            {
                proj.Part.CreateRelationship(UriHelper.ResolvePartUri(proj.Uri, Uri), TargetMode.Internal, schemaRelVbaSignature);
            }
        ...
        {
            //MS-OVBA 2.4.2
            var enc = Encoding.GetEncoding(proj.CodePage);
            var bw = new BinaryWriter(new MemoryStream());
            ...
            {
                string[] lines = module.Code.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries);
            ...
            byte[] buffer = (bw.BaseStream as MemoryStream).ToArray();
            var hp = MD5.Create();
